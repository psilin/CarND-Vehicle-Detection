...
class RingBuffer:
    ...
    def add(self, rectangles):
        if len(self.rectangles_list) > self.length: self.rectangles_list.pop(0)
...
def prepareData(veh_dir, non_veh_dir, color_space, hog_channel, orient):
    ...
    hog_features_veh = extract_hog_features(vehicle_files, cspace=color_space, hog_channel=hog_channel, orient=orient)
    ...
    hog_features_non_veh = extract_hog_features(non_vehicle_files, cspace=color_space, hog_channel=hog_channel, orient=orient)
    ...
    y = np.hstack((np.ones(len(hog_features_veh)), np.zeros(len(hog_features_non_veh))))
...
def prepare_svm(X_train, y_train, X_test, y_test, X_scaler, kernel='linear', C=10):
